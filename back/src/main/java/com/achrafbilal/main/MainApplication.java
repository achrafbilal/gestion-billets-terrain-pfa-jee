...
public class MainApplication implements ApplicationRunner {
	...
	public void run(ApplicationArguments args) {
		...
		for (int i = 0; i < 20; i++) {
			...
			c.add(Calendar.DATE, 30);
			ticket.setUserId((long) 31);
			// ticket.setUserId(users.get(random.nextInt(users.size() - 1)).getId());

			tickets.add(ticketRepo.save(ticket));
		}

		System.out.println(String.format("%d zone created + %d user created + %d ticket created", zones.size(),
				users.size(), tickets.size()));
	}
}
